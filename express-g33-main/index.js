...
app.put("/EditarUsuario", (request, response) => {
  ...
  const datoNuevo = { nombre: request.body.nombre };
  const filtro = { cedula: request.body.cedula };

  UsuarioModelo.findAndUpdate(filtro, datoNuevo, function (error, documento) {

    if (error) {
      response.send('Error al Editar');
    } else {
      response.send("El usuario de editado super bien");
    }
